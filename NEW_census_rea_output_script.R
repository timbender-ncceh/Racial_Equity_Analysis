...
c("sum(", paste(paste("out5$", colnames(out4)[8:ncol(out4)], sep = ""), sep = "[i], ", 
...
out5$calc_type[!grepl(pattern = "\\-", 
                      x = out5$group_calculation)] <- "sum_all"

out5$calc_type[is.na(out5$calc_type) & 
